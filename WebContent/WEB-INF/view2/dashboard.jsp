	...
	<div id="content" class="container">
		...
		<div class="row-fluid">
			<ul class="nav nav-tabs">
				...
				<li id="sjtj_li"><a href="#tab2" data-toggle="tab">数据统计</a></li>
		...
		<!-- <div class="easyui-window " id="extendedcase_window" title="各地区超期案件" closed="true" style="overflow-x: hidden;width:200px;height:400px;display:none;">
			<div id="extendedcase" ></div>
		</div> -->
...
$(function(){
	...
	$.ajax({
		url:"<%=request.getContextPath() %>/count/countFindExtend.do",
		type:'post',
	...
	initCharts1(startTime,endTime);
	initCharts2(startTime,endTime);
...
function initCharts1(startTime,endTime){
	$.ajax({
		type:'get',
		...
		success:function(data){
			...
			initChart("column_org_processNode",xtext,"各不动产登记中心办理案件流程统计",''/* "受理信息" */,"",columnSeries);
		}
	});
}
function initCharts2(startTime,endTime){
	$.ajax({
		type:'get',
		url:'<%=request.getContextPath() %>/count/findAll.do',
		data:{startTime:startTime,endTime:endTime},
...
function initGrid(startTime,endTime){
	$('#gd').datagrid({
		...
		columns:[[
            ...
            {title:'登记中心',field:'dept',width:'50%',sortable:true,
            ...
            {title:'流程节点',field:'processNode',width:'25%' },
            {title:'合计',field:'total',width:'20%' }
...
function initChart(target,xAxisText,title,title2,ytitle,series){
	new Highcharts.Chart({
        ...
        xAxis: { //x轴
			...
			labels:{y:18,rotation: -20},  //x轴标签位置：距X轴下方18像素
        },
		yAxis: {  //y轴
...
function initExtendedCase(startTime,endTime){
	...
	$("#extendedCase").datagrid({
		title:'各登记中心超期案件统计',//标题
	})
	$("#extendedCase,#extendedcase").datagrid({
		//title:'超期案件统计',//标题
		width:'100%',
		height:'100%',
		nowrap: false, //是否在一行显示数据
		...
		fitColumns: true,
		columns:[[
            {title:'登记中心',field:'orgName',width:'67%',sortable:true},
            {title:'合计',field:'total',width:'25%' }
		]] ,
...
function loadDetail(rowIndex,rowData){
	var scrollTop = null; 
	if(document.all){
		scrollTop = window.document.documentElement.scrollTop+100;
	}else{
		scrollTop = window.pageYOffset || window.document.body.scrollTop;
...
function loadFlow(dataObj){
	...
	$('#caseFlow').window({left:$(window).width()/2-550, top:scrollTop+"px"});
...
function reload(){
	...
	var startTime = $("#startTime1").datebox("getValue");
	var endTime = $("#endTime1").datebox("getValue");
	//initContent(startTime,endTime);
	$(".box_div").remove();
	initContent1(startTime,endTime);
	initContent2(startTime,endTime);
	$(".mymask").hide();
}

function initContent1(startTime,endTime){
	$.ajax({
		type:"get",
...
function initContent2(startTime,endTime){
	$.ajax({
		type:"get",
		url:"<%=request.getContextPath()%>/case/countCaseDetial.do",
		data:{startTime:startTime,endTime:endTime},
